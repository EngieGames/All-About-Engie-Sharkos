...
public class EngieSharkoRARE2TAPProcedure {
			...
			if ((sourceentity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == AaeSharkosModItems.ENGIE_COOKIE.get()) {
				if (Math.random() >= 0.9) {
					if (world instanceof ServerLevel _level) {
						Entity entityToSpawn = new EngieSharkoRARE2TamedEntity(AaeSharkosModEntities.ENGIE_SHARKO_RARE_2_TAMED.get(), _level);
