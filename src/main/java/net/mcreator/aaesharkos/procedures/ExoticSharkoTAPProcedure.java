...
public class ExoticSharkoTAPProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, Entity sourceentity) {
		if (entity == null || sourceentity == null)
			return;
		if (!world.isClientSide()) {
			if ((sourceentity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == AaeSharkosModItems.EXOTIC_COOKIE.get()) {
				if (Math.random() >= 0.85) {
					...
					if (sourceentity instanceof Player _player) {
						ItemStack _stktoremove = new ItemStack(AaeSharkosModItems.EXOTIC_COOKIE.get());
